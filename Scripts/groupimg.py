...
ap.add_argument("-f", "--folders", required=True, help="path to image folder")
ap.add_argument("-k", "--kmeans", type=int, default=2, help="how many groups")
...
for folder in listdirs(folders):
    print(folders+folder)
    for files in types :
        var = imagePaths.extend(sorted(glob.glob(folders+folder+"/"+files)))
    ...
    for i in range(k.k) :
        try :
            os.makedirs(folders+folder+'/'+str(i+1).zfill(nfolders))
        except :
    ...
    for i in range(len(k.cluster)):
        action(k.end[i], folders+folder+"/"+str(k.cluster[i]+1).zfill(nfolders)+"/")
